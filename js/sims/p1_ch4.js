export const p1_ch4_sims = {
  setup: (sketch, vizType) => {
    ...
    sketch.startCollision = () => {
      ...
      sketch.initialKE = 0.5 * sketch.m1_col * sketch.v1 * sketch.v1 + 0.5 * sketch.m2_col * sketch.v2 * sketch.v2;
    };
    ...
    switch (vizType) {
      case "banking_road": {
        ...
        if (isSkidding && sketch.frameCount % 2 === 0) {
          ...
          sketch.noStroke();
          sketch.circle(-35 - sketch.random(10), -5 - sketch.random(8), sketch.random(6, 12));
          sketch.circle(35 + sketch.random(10), -5 - sketch.random(8), sketch.random(6, 12));
        }
        ...
        if (Math.abs(L_f) > 1.5) {
          ...
          sketch.fill("#f59e0b");
          sketch.text(L_f > 0 ? "F_f (inward)" : "F_f (outward)", cx - L_f + (L_f > 0 ? -45 : 10), cy - 4);
        }
        ...
        if (isSkidding) {
          ...
          sketch.text("⚠️ VEHICLE SKIDDING: TIRE SLIP! ⚠️", sketch.width / 2, 62);
      ...
      case "pulley_system": {
        ...
        if (sketch.pulleyRunning) {
          ...
          // Limit limits
          if (sketch.y1 > 220 || sketch.y2 > 220 || sketch.y1 < 70 || sketch.y2 < 70) {
            sketch.pulleyRunning = false;
            sketch.vel1 = 0;
            sketch.vel2 = 0;
          }
        ...
        // Force vector on m1
        p1_ch4_sims.drawArrow(sketch, px - r, sketch.y1 + w1/2, px - r, sketch.y1 + w1/2 - T * 1.5, "#10b981", 2); 
        p1_ch4_sims.drawArrow(sketch, px - r, sketch.y1 + w1/2, px - r, sketch.y1 + w1/2 + curM1 * g * 0.8, "#ef4444", 2); 
        ...
        // Force vector on m2
        p1_ch4_sims.drawArrow(sketch, px + r, sketch.y2 + w2/2, px + r, sketch.y2 + w2/2 - T * 1.5, "#10b981", 2); 
        p1_ch4_sims.drawArrow(sketch, px + r, sketch.y2 + w2/2, px + r, sketch.y2 + w2/2 + curM2 * g * 0.8, "#ef4444", 2); 
      ...
      case "collision_lab": {
        ...
        if (sketch.collisionRunning) {
          ...
          sketch.currentKE = 0.5 * curM1 * sketch.v1 * sketch.v1 + 0.5 * curM2 * sketch.v2 * sketch.v2;
          ...
          if (!sketch.collided && (sketch.x1 + w1/2 >= sketch.x2 - w2/2)) {
            ...
            sketch.v1 = (curM1 * u1 + curM2 * u2 - curM2 * curE * (u1 - u2)) / (curM1 + curM2);
            sketch.v2 = (curM1 * u1 + curM2 * u2 + curM1 * curE * (u1 - u2)) / (curM1 + curM2);
          }
        ...
        if (sketch.collisionRunning) {
          p1_ch4_sims.drawArrow(sketch, sketch.x1, roadY - w1 - 10, sketch.x1 + sketch.v1 * 6, roadY - w1 - 10, "#0ea5e9", 2);
          p1_ch4_sims.drawArrow(sketch, sketch.x2, roadY - w2 - 10, sketch.x2 + sketch.v2 * 6, roadY - w2 - 10, "#f59e0b", 2);
        }
        ...
        sketch.textSize(9);
        sketch.textStyle(sketch.BOLD);
        sketch.text("Kinetic Energy Conservation (J)", chartX + 15, chartY - 33);
        sketch.textStyle(sketch.NORMAL);
        ...
        // Initial KE bar
        sketch.fill("#94a3b8");
        sketch.rect(chartX + 110, chartY - 25, sketch.initialKE * scaleKE, 8, 2);
        ...
        sketch.fill("#ef4444");
        sketch.rect(chartX + 110 + b1_w + b2_w, chartY - 12, lostKE * scaleKE, 8, 2); 
        
        sketch.fill("#1e293b");
        sketch.text(`Initial E_k: ${sketch.initialKE.toFixed(1)} J`, chartX + 15, chartY - 19);
        sketch.text(`Current E_k: ${activeTotalKE.toFixed(1)} J`, chartX + 15, chartY - 6);
