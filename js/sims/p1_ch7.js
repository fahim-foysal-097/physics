  ...
  draw: (sketch, vizType) => {
    ...
    switch (vizType) {
      case "poissons_ratio": {
        ...
        let str = sketch.sin(sketch.frameCount * 2.0) * 16; // smooth oscillatory strain

        p1_ch7_sims.drawGrid(sketch);

        // --- Positive Poisson's Ratio (Slate/Green Specimen) ---
        sketch.push();
        sketch.translate(mid / 2 - 20, sketch.height / 2 - 10);

        let origW = 60;
        let origH = 120;
        let deltaH = str;
        let deltaW = -deltaH * 0.35; // Positive Poisson: contracts sideways

        // Draw upper and lower steel grips
        sketch.fill("#475569");
        sketch.stroke("#1e293b");
        sketch.strokeWeight(1.5);
        sketch.rect(-origW/2 - 6, -origH/2 - 15, origW + 12, 15, 2); // Top grip
        sketch.rect(-origW/2 - 6, origH/2 + deltaH, origW + 12, 15, 2); // Bottom grip

        // Draw active specimen (Vibrant semi-transparent teal)
        sketch.fill("rgba(16, 185, 129, 0.25)");
        sketch.stroke("#059669");
        sketch.strokeWeight(2.5);
        sketch.rect(-(origW + deltaW)/2, -origH/2, origW + deltaW, origH/2 + (origH/2 + deltaH), 4);

        // Draw dynamic interior lattice grid!
        sketch.stroke("rgba(5, 150, 105, 0.4)");
        ...
        if (deltaH > 2) {
          p1_ch7_sims.drawArrow(sketch, 0, origH/2 + deltaH, 0, origH/2 + deltaH + 18, "#ef4444", 2); // Tension
          p1_ch7_sims.drawArrow(sketch, curW/2, 0, curW/2 - 12, 0, "#0ea5e9", 2); // Contraction lateral
          p1_ch7_sims.drawArrow(sketch, -curW/2, 0, -curW/2 + 12, 0, "#0ea5e9", 2); 
        }
        ...
        sketch.text("Positive Ratio (σ = 0.35)", -55, -origH/2 - 25);
        sketch.pop();

        // --- Negative Poisson's Ratio (Auxetic Material - widens when stretched!) ---
        sketch.push();
        sketch.translate(mid + mid / 2 - 20, sketch.height / 2 - 10);

        let deltaWNeg = deltaH * 0.30; // Auxetic Poisson: expands sideways!

        // Draw upper and lower steel grips
        sketch.fill("#475569");
        sketch.stroke("#1e293b");
        sketch.strokeWeight(1.5);
        sketch.rect(-origW/2 - 6, -origH/2 - 15, origW + 12, 15, 2); // Top
        sketch.rect(-origW/2 - 6, origH/2 + deltaH, origW + 12, 15, 2); // Bottom

        // Draw active specimen (Vibrant gold auxetic rubber)
        sketch.fill("rgba(245, 158, 11, 0.22)");
        sketch.stroke("#d97706");
        sketch.strokeWeight(2.5);
        sketch.rect(-(origW + deltaWNeg)/2, -origH/2, origW + deltaWNeg, origH/2 + (origH/2 + deltaH), 4);

        // Draw dynamic interior lattice grid!
        sketch.stroke("rgba(217, 119, 6, 0.4)");
        sketch.strokeWeight(1.0);
        let curWNeg = origW + deltaWNeg;
        
        // Vertical grid lines deforming
        for (let col = 1; col < columns; col++) {
          let gx = -curWNeg / 2 + (curWNeg / columns) * col;
          sketch.line(gx, -origH/2, gx, origH/2 + deltaH);
        }
        // Horizontal grid lines deforming
        for (let row = 1; row < rows; row++) {
          let gy = -origH/2 + (curH / rows) * row;
          sketch.line(-curWNeg / 2, gy, curWNeg / 2, gy);
        }

        // Strain vector force arrows
        if (deltaH > 2) {
          p1_ch7_sims.drawArrow(sketch, 0, origH/2 + deltaH, 0, origH/2 + deltaH + 18, "#ef4444", 2); // Tension
          p1_ch7_sims.drawArrow(sketch, curWNeg/2, 0, curWNeg/2 + 12, 0, "#10b981", 2); // Expansion lateral
          p1_ch7_sims.drawArrow(sketch, -curWNeg/2, 0, -curWNeg/2 - 12, 0, "#10b981", 2); 
        }
        ...
        sketch.textSize(10);
        sketch.textStyle(sketch.BOLD);
        sketch.text("Negative Ratio (σ = -0.30)", -55, -origH/2 - 25);
        sketch.pop();
        
        p1_ch7_sims.drawPills(sketch, "Poisson's Ratio Grid Deformation Rig", [
          `σ = - ε_lateral / ε_longitudinal`,
          `ε_long = ${((origH + deltaH - 120)/120).toFixed(3)}`,
          `ε_pos_lat: ${((origW + deltaW - 60)/60).toFixed(3)}`,
          `ε_neg_lat: ${((origW + deltaWNeg - 60)/60).toFixed(3)}`
        ]);
        break;
      }
      ...
      case "stress_strain_curve": {
        ...
        let getMaterialStrain = (m, l) => {
          ...
          let e_el = st / Y;
          if (m === 0 && l > plasticTh) {
            return e_el + Math.pow((l - plasticTh) / 70, 2) * 0.010;
        ...
        let wireColor = mat === 0 ? "#94a3b8" : mat === 1 ? "#f59e0b" : "#e2e8f0";
        ...
        if (!sketch.wireBroken) {
          ...
          for (let c = 0; c < cols; c++) {
            for (let r = 0; r < rows; r++) {
              ...
              if (r < rows - 1) sketch.line(ax, ay, ax + (r >= 2 ? slipFactor : 0), ay + atomSpacing * atomStretch); // vertical
            }
          }
          ...
          for (let c = 0; c < cols; c++) {
            for (let r = 0; r < rows; r++) {
              ...
              let ay = lensY - 22 + r * atomSpacing + dirY + sketch.random(-2, 2);
        ...
        sketch.text("Steel", mapX(getMaterialStrain(0, 170)) + 3, mapY(170) + 3);
        ...
        sketch.text("Copper", mapX(getMaterialStrain(1, 135)) + 3, mapY(135) + 3);
        ...
        p1_ch7_sims.drawPills(sketch, `Automated Wire Tensile Tester (${matName})`, [
          ...
          sketch.wireBroken ? "SNAP! Snapped" : load > plasticThreshold ? "Plastic Region" : "Elastic Region"
        ]);
        break;
      }
      ...
      case "terminal_velocity_stokes": {
        ...
        if (sketch.stokesRunning) {
          let vScale = 3e4; 
          p1_ch7_sims.drawArrow(sketch, cx, sketch.sphereY, cx, sketch.sphereY + gravityForce * vScale, "#ef4444", 2);
          p1_ch7_sims.drawArrow(sketch, cx, sketch.sphereY, cx, sketch.sphereY - buoyancy * vScale, "#0ea5e9", 2);
          p1_ch7_sims.drawArrow(sketch, cx, sketch.sphereY, cx, sketch.sphereY - (buoyancy + drag) * vScale, "#f59e0b", 2.0);
          ...
          sketch.fill("#f59e0b");
          sketch.text("F_drag", cx - 38, sketch.sphereY - (buoyancy + drag) * vScale + 5);
        }
