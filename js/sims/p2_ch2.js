export const p2_ch2_sims = {
  setup: (sketch, vizType) => {
    const defaults = {
      ...
      electric_field: { q1: 5, q2: -5, resolution: 25, probeX: 0, probeY: 0, t: 0 },
      sphere_field_graph: { Q: 10, R: 40, probeR: 80 },
      dipole_torque: { E: 5, theta: 45, fieldAngle: 0, isPlaying: false, angularVelocity: 0 },
  ...
  draw: (sketch, vizType) => {
    const drawCharge = (x, y, q, radius) => {
      let c = q > 0 ? sketch.color(239, 68, 68) : (q < 0 ? sketch.color(59, 130, 246) : sketch.color(156, 163, 175));
      sketch.noStroke();
      for (let i = 4; i > 0; i--) {
    ...
    switch (vizType) {
      case "coulombs_law": {
        ...
        if (q1 !== 0 && q2 !== 0) {
          let fColor = isAttractive ? sketch.color(16, 185, 129) : sketch.color(245, 158, 11);
          let dir1 = isAttractive ? 1 : -1;
          let dir2 = isAttractive ? -1 : 1;

          let forceVec1 = sketch.createVector(dir1 * Math.min(forceMag, 100), 0);
          let forceVec2 = sketch.createVector(dir2 * Math.min(forceMag, 100), 0);
      ...
      case "electric_field": {
        ...
        for (let x = startX; x < sketch.width / 2; x += res) {
          for (let y = startY; y < sketch.height / 2; y += res) {
            ...
            if (eMag > 0.1) {
              ...
              drawArrow(x, y, E, sketch.color(14, 165, 233, alpha + pulse), 1.5);
            }
          }
        }
        ...
        let Ep = p5.Vector.add(fieldFromCharge(probe, p1, q1), fieldFromCharge(probe, p2, q2));
        ...
        if (Ep.mag() > 0.001) {
          drawArrow(probe.x, probe.y, Ep.copy().setMag(probeLen), sketch.color(20, 184, 166), 3);
        }
        ...
        let Q = sketch.Q ?? 10;
        let R = sketch.R ?? 40;
        let probeR = sketch.constrain(sketch.probeR ?? 80, 0, sketch.width - 280);
        ...
        const c = Q > 0 ? sketch.color(239, 68, 68) : sketch.color(59, 130, 246);

        // Left: charged conducting sphere
        sketch.noStroke();
        for (let r = R; r > 0; r -= 2) {
          sketch.fill(c.levels[0], c.levels[1], c.levels[2], sketch.map(r, 0, R, 255, 40));
        ...
        sketch.rect(graphStsartX, -150, rAxisSphere, 250);
        ...
        sketch.line(graphStartX + rAxisSphere, 100, graphStartX + rAxisSphere, -150);
        ...
        sketch.text(`E = ${eProbe.toFixed(1)}  V = ${vProbe.toFixed(1)}`, probeX, -80);
      ...
      case "dipole_torque": {
        ...
        if (sketch.isPlaying) {
          let alpha = -0.01 * E * sketch.sin(sketch.radians(theta - fieldAngle));
        ...
        sketch.text(`τ ∝ pE sin(θ - φ) = ${Math.abs(torqueMag).toFixed(1)}`, 0, 160);
      ...
      case "capacitor": {
        ...
        // Field lines
        const fieldLineCount = sketch.max(5, Math.floor((plateHeight * V) / 60));
        ...
        for (let i = 0; i < fieldLineCount; i++) {
          ...
          sketch.triangle(rightX - 7, y, rightX - 14, y - 4, rightX - 14, y + 4);
          sketch.stroke(14, 165, 233, 180);
        }
        ...
        sketch.arc(leftX + plateWidth / 2, -plateHeight / 2, 30, 18, 0, sketch.PI);
        sketch.arc(rightX - plateWidth / 2, -plateHeight / 2, 30, 18, sketch.PI, sketch.TWO_PI);
        sketch.arc(leftX + plateWidth / 2, plateHeight / 2, 30, 18, sketch.PI, sketch.TWO_PI);
        sketch.arc(rightX - plateWidth / 2, plateHeight / 2, 30, 18, 0, sketch.PI);
        ...
        sketch.text(`U = 1/2 CV² : ${U.toFixed(2)}`, -125, plateHeight / 2 + 88);
